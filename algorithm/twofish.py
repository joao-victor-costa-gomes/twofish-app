# CÓDIGO RETIRADO DE: https://github.com/d4rkvaibhav/twofish_python/
...
if __name__ == "__main__":
    ...
    typ=input("\nEnter the type (Encrypt/Decrypt) : ")

    key=input("\nEnter the key 128 bit : ")

    hexa_key = texto_para_hex(key)
    print(f"\nSua chave em hexadecimal : {hexa_key}")

    key=hexa_key.zfill(32)

    if(typ.lower()=="encrypt"):
        plaintext=input("\nEnter the plaintext 128 bit : ")

        hexa_plaintext = texto_para_hex(plaintext)
        print(f"\nSua mensagem em hexadecimal: {hexa_plaintext}")

        plaintext=hexa_plaintext.zfill(32)

        print("\nThe Ciphertext is : ",end=" ")

        print(encrypt(plaintext,key))

    else:
        Ciphertext=input("\nEnter the Ciphertext 128 bit : ")
        print("\nThe Decoded plaintext is : ",end=" ")
        mensagem_decodificada = decrypt(Ciphertext,key)
        print(hex_para_texto(mensagem_decodificada))
